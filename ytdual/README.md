...
`normalizeSegments` · `pickTrack` · `seedRawLines` · `mergeTranslated` ·
`findLine` · `requestTranslation` 을 직접 호출합니다. 사본을 두지 않으므로
여기서 본 동작이 곧 확장의 동작입니다. 상단 배지가 연결 상태를 보여줍니다.

볼 수 있는 것:

1. **자막과 재생 지점** — 로컬 fixture 13개 중 선택. 시간 정규화를 껐다 켜면
   자동생성 자막의 rolling 겹침이 사라지는 것이 수치로 보입니다.
2. **번역 실행** — 재생 지점의 청크를 번역하고 초 단위로 카운트합니다.
   누적 표에 지연·토큰·reasoning·빈 번역·구멍·최장줄이 쌓이고 중앙/최소/최대가
   나옵니다.
3. **번역 결과** — 빈 번역은 빨간 테두리, 85자 초과는 노란 테두리.
4. **오버레이 미리보기** — 실제 `overlay.css` 와 실제 `findLine()` 으로 그립니다.
   슬라이더를 끌어 자막이 비는 순간이 있는지 확인할 수 있습니다.

`전체 번역` 은 동시성 8로 영상 전체를 번역합니다 — 실제 비용이
