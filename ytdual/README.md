...
```bash
npm run dev        # 워커 (:8787)
npm run harness    # 하네스 (:8788) → http://127.0.0.1:8788
```

페이지는 `ytdual/extension/content.js` **원본을 그대로 로드해** 그 안의 함수를
직접 호출합니다 — `normalizeSegments` · `makeJobs` · `pickJob` · `splitJobAt` ·
`seedRawLines` · `mergeTranslated` · `findLine` · `requestTranslation` · `pickTrack`.
사본이 없으므로 여기서 본 동작이 곧 확장의 동작입니다. 상단 배지가 연결 상태를
보여줍니다.

1. **자막** — 로컬 fixture 선택. 시간 정규화를 껐다 켜면 자동생성 자막의
   rolling 겹침이 사라지는 것이 수치로 보입니다.
2. **번역** — 재생 위치를 옮기고 `여기서 시작`. 확장과 같이 8개를 동시에 돌려
   그 지점부터 번역합니다. 게이지는 **그 지점 자막이 뜨기까지**를 재고 3초에
   눈금이 있습니다. 세션이 도는 중에 위치를 옮기고 `여기로 시킹`을 누르면
   무관해진 요청이 취소되는 것과 그 뒤 지연을 볼 수 있습니다.
3. **화면** — 실제 `overlay.css` 와 실제 `findLine()` 으로 그립니다. 비는 순간이
   있으면 여기서 드러납니다.
4. **줄** — 재생 지점 주변 40줄. 원문 대기(회색) / 번역됨(초록) / 빈 번역(빨강).

`캐시 우회`를 끄면 같은 구간을 다시 눌렀을 때 KV 캐시가 히트해 0초에 돌아옵니다.
전체 번역은 실제 비용이 발생하므로 짧은 fixture로 먼저 시험하세요.
